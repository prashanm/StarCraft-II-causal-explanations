...
from rl.pre_processing import is_spatial_action, NUM_FUNCTIONS, FLAT_FEATURES, SFEATURES, MFEATURES
...
class FullyConv():
  ...
  def build(self, screen_input, minimap_input, flat_input):
    ...
    screen_emb = self.embed_obs(screen_input, SFEATURES,
                                self.embed_spatial)
    minimap_emb = self.embed_obs(minimap_input, MFEATURES,
                                 self.embed_spatial)
