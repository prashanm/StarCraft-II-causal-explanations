...
from rl.pre_processing import is_spatial_action, stack_ndarray_dicts, is_queued_action
...
class A2CRunner():
  def __init__(self,
    ...
    self.preproc = Preprocessor(self.envs.observation_spec()[0], self.minimap_size)
  ...
  def _summarize_episode(self, timestep):
    ...
    if self.summary_writer is not None:
      ...
      winp = ((self.win_perc) / (self.episode_counter + 1))*100
      drawp = ((self.draw_perc) / (self.episode_counter + 1))*100
      ...
      summary.value.add(tag='sc2/lose', simple_value=losep)

      self.summary_writer.add_summary(summary, self.episode_counter)

    print("episode %d: score = %f , reward = %e , total wins = %g" % (self.episode_counter, score, timestep.reward, self.win_perc))
