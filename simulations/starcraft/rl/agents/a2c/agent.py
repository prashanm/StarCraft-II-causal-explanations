...
class A2CAgent():
  ...
  def build(self, static_shape_channels, resolution, scope=None, graph=None, reuse=None):
    self.tf_star_graph = graph
  ...
  def train(self, obs, actions, returns, advs, summary=False):
    ...
    res = self.starsess.run(ops, feed_dict=feed_dict)
    agent_step = self.train_step
    self.train_step += 1

    if summary:
      return (agent_step, res[1], res[-1], feed_dict)
  ...
  def step(self, obs):
    ...
    """
    feed_dict = self.get_obs_feed(obs)
    return self.starsess.run([self.samples, self.value], feed_dict=feed_dict)

  def get_value(self, obs):
    return self.starsess.run(
        self.value,
        feed_dict=self.get_obs_feed(obs))

  def init(self):
    self.starsess.run(self.init_op)
  ...
  def save(self, path, step=None):
    ...
    self.saver.save(self.starsess, ckpt_path, global_step=step)

  def load(self, path):
    ckpt = tf.train.get_checkpoint_state(path)
    self.saver.restore(self.starsess, ckpt.model_checkpoint_path)
